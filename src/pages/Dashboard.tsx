...
const Dashboard = () => {
  ...
  const partsNeedingAttention = mockParts.filter(p => p.health < 75)
    .sort((a, b) => a.health - b.health);
    
  return (
    <div className="container px-4 py-6 mx-auto max-w-7xl">
      <h1 className="text-3xl font-bold mb-6">Vehicle Vitality Monitor</h1>
              ...
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {recommendations.length > 0 && (
                    <Badge variant="outline" className="ml-2 bg-auto-warn text-black">
                    ...
                    {recommendations.map((rec) => (
                      <div 
                        ...
                        className={`p-3 rounded-md border flex items-start gap-3 ${
                          ...
                          rec.urgency === 'medium' ? 'bg-amber-50 border-amber-200' : 
                          'bg-blue-50 border-blue-200'
                        }`}
                      >
                        <AlertTriangle 
                          className={`h-5 w-5 mt-0.5 ${
                            rec.urgency === 'high' ? 'text-auto-danger' : 
                            rec.urgency === 'medium' ? 'text-auto-warn' : 
                            'text-auto-blue'
                          }`} 
                        />
                        <div>
            ...
            {partsNeedingAttention.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {partsNeedingAttention.map(part => (
                  <VehiclePartStatus key={part.id} part={part} />
                ))}
              </div>
            )}
          </div>
